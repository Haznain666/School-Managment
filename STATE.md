...
**Last updated:** 2026-08-13 (**Sprint 10.5: design system + application shell built — §5z**)

> ▶ **NEXT: adopt `PageHeader` and `Table` across the screens.** Sprint 10.5's
> deliverables A, B and D are done and committed on
> `claude/crm-ui-design-colors-2fe2d4` (§5z): the derived token layer, 11 new
> primitives, the 8 old ones retrofitted, 5 SVG chart components, all five
> shells rebuilt with icons and a mobile drawer, and workbenches at
> `/design-system` and `/design-system/shell`. **Individual screens are still
> untouched** — `PageHeader` exists and almost nothing uses it. Read §5z first;
> the remaining work is mechanical, and the rule that keeps it correct is
> `npm run check-theme`.
...
Run with `/impeccable` as the user instructed. Five commits on
...
1. **Adopt `PageHeader` across the screens.** It exists and almost nothing uses
   it; every screen still writes its own heading, and they disagree on size,
   spacing and where the action button goes. This is the cheapest remaining
   coherence win and needs no new thinking.
2. **Wire the charts to real dashboard queries** (deliverable C). The
   income/expense tiles must use `StatTile`'s `unavailable` prop until Sprint
   13.5's ledger exists — **not a zero**.
3. **Replace the improvised tables** with `components/ui/Table`, which is what
   fixes numeric alignment and the sticky header across the list screens.
