...
> palette across 145 files, and **every table in the product on the `Table`
> primitive**. Read §5z for how the 27-file table rewrite was verified — the
> same two checks are worth reusing for any future bulk change.
...
Run with `/impeccable` as the user instructed. Ten commits on
...
1. **Wire the charts to real dashboard queries** (deliverable C) — the only
   Sprint 10.5 deliverable still open. The
