...
**Last updated:** 2026-08-14 (**Sprint 10.5: design system, shell and the screen-level pass — §5z**)

> ▶ **NEXT: wire the charts to real dashboard queries (deliverable C).** It is
> the only deliverable left. A, B, D and the screen-level pass are done on
> `claude/crm-ui-design-colors-2fe2d4` (§5z): the derived token layer, 11 new
> primitives, 5 SVG charts, all five shells rebuilt with icons and a mobile
> drawer, `PageHeader` on 49 screens, and 1155 hardcoded colours migrated onto
> the palette across 145 files. **`Table` adoption is partial by design** — 8
> files converted, 25 held back because they carry selection, sorting or inline
> forms, which are behaviour. Read §5z for that boundary before continuing.
...
Run with `/impeccable` as the user instructed. Nine commits on
...
| C — dashboard visualisation | ⚠️ components built; **wired to no real screen** |
...
- **The worktree now has its own `node_modules`** (from installing
  `lucide-react`). This incidentally kills the §5f build hazard: no
  `.claude/worktrees/node_modules` stub is created, and the second build no
  longer fails. Confirmed after a full build.
...
**`Table`: 8 files.** Scope was deliberately bounded to read-only list tables.

### ⚠️ `Table` adoption is PARTIAL, and the boundary is deliberate

**25 files still hold a raw `<table>`.** They were not skipped for lack of
time; each carries something a mechanical conversion would change:

| Why held back | Files |
| --- | --- |
| Selection / sort logic | `UserTable`, `ChallanTable`, `MarksEntry`, `ApplicationTable`, `SchoolUsersTable`, `StaffDetailPanel`, `ChallanGenerator`, parent fees |
| Inline form controls in cells | `FeeReports`, `FeeTypeManager`, `StaffManager`, `LeaveManager`, `SalaryComponentManager`, `PayrollRunManager`, `ExamScheduler`, `GradingSchemeEditor`, `StudentImporter`, `StudentTable`, `PromotionRunner`, `FamilyVouchers`, `AttendanceReports` |
| Not a list table | `TimetableGrid`, `TimetableBuilder`, `PermissionMatrix`, `FeeStructureMatrix` |
| Two tables in one file | `fees/challans/[challanId]` — the transform rewrote both tables' cells but only one wrapper; reverted |

Sprint 10.5 says a behaviour change is a defect, and a checkbox column or a
sort handler is behaviour. **Each of these needs its own conversion and its own
re-verification**, not a regex. They are all on the tokens already, so they
match the school's palette — what they lack is the sticky header, the numeric
alignment and the shared scroll container.

### What to do next

1. **Finish the eight remaining `PageHeader` pages** — trivial, listed above.
2. **Convert the held-back tables one at a time**, starting with `ChallanTable`
   and `UserTable` (highest traffic), re-testing selection after each.
3. **Wire the charts to real dashboard queries** (deliverable C). The
   income/expense tiles must use `StatTile`'s `unavailable` prop until Sprint
   13.5's ledger exists — **not a zero**.
