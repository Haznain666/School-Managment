...
**Last updated:** 2026-08-13 (**Sprint 10.5 foundation built — §5z**)

> ▶ **NEXT: roll the design system out across the five shells.** Sprint 10.5's
> foundation is built and committed on `claude/crm-ui-design-colors-2fe2d4`
> (§5z): the derived token layer, 11 new primitives, the 8 old ones retrofitted,
> 5 SVG chart components, and a workbench at `/design-system`. **Deliverable B
> — the application shell — is not started**, and no existing screen has been
> touched. Read §5z before continuing; the rollout is mechanical but the rule
> that makes it work is `npm run check-theme`.
...
## 5z. Sprint 10.5 — design system foundation built 2026-08-13

Run with `/impeccable` as the user instructed. Three commits on
...
| B — the application shell (sidebar, headers, states) | ❌ **not started** |
| C — dashboard visualisation | ⚠️ components built; **wired to no real screen** |
| D — the charting decision | ✅ settled and implemented |
| E — verification | ✅ done for the components; not for real screens |

**No existing screen was touched.** The 105 components still render exactly as
they did, minus the colour changes that come free from the retrofitted
primitives. That is deliberate — the foundation had to be provably correct
before anything was rebuilt on it — but it means the product does not *look*
overhauled yet. Deliverable B is where that becomes visible.
...
maroon). **It 404s outside development** (verified: `status: 404` in the
prerender meta), so it ships in nothing a school can reach. It exists because
...
1. **Deliverable B, the application shell** — this is what makes the overhaul
   visible. `PortalSidebar` already consumes `secondary`/`accent` correctly and
   needs icons per nav item, grouping and a collapse; page headers need the
   `Breadcrumb` + primary-action slot that now exist.
2. Then wire the charts to real dashboard queries (deliverable C). The
