...
which is step two, reachable only by uploading the file you are asking about.
The answer to "what do you want in it" sat behind the thing it was the answer
to. §5bu.

🔴 **The sample sheet is generated from `IMPORT_FIELDS`, never typed.**
`lib/student-import-sample.ts` throws at module load if a field has no column,
and `npm run check-import-sample` (25 assertions, **in CI**) reads the file's own
bytes back through `parseCsv`, `suggestColumnMap` and `validateRow` and requires
a bijection. **If you add or rename an import field, run that check** — every
way this rots is silent: the file still downloads, still uploads, and the columns
merely arrive unmatched, which reads as poor guessing rather than a stale sample.

⚠ **The green-build list is now thirteen, not twelve.** `check-import-sample`
is in `CLAUDE.md`'s list and in `.github/workflows/ci.yml`, added to both
together as that file requires.

⚠ **The sample's headings are the importer's own aliases**, which is what makes
a file built on it arrive fully mapped. That coupling is deliberate and is the
thing the check protects. `b-form / cnic` was added to that field's alias list
for exactly this reason — not to satisfy the check, but so the heading the
product hands a school is one the product recognises outright.

Previously: **Sprint 30 — the desk that reached nobody, and the campus the
header never named — §5bt.**
...
**7. Rating principals and branch admins are two School Admin settings.**
...
> Admin**.
>
> *Mark branch admins' progress?* **Yes / No**. If Yes, *who rates them* — select
> all that apply: **School Admin** (default), **the Branch Admin themselves**,
> **Principal**.

With *No*, that role carries no KPIs to rate. A self-rating is allowed where
selected; rule 5 still applies, so a more senior rating of the same KPI outranks
it.
...
- **Proposed permission keys, mirroring CRUD plus the view-only score:**
  ...
  | `kpis.rate.<role>` | enter or change a rating and its comment for **that target role**, within scope | see the grid below |
  | `kpis.overall` | the yearly overall score **only** | Accountant (Finance), plus everyone holding `kpis.read` |

  **Rating is one key per target role** (rule 7a), so the existing matrix *is*
  the who-rates-whom grid and the School Admin manages it with
  `permissions.manage`, which they always keep. Proposed keys and defaults:
  ...
  **Principals and branch admins get no rate key** — rule 7's two settings are
  ...
  it, and "may rate a principal" would then have two answers.

  A staff member's **own** scores need no key, the way their own payslip needs
  none. Editing a KPI's *definition* sits with `kpis.create`, not a rate key, so
  a coordinator's rating right never reaches it. The coordinator's "read and
  update" is `kpis.read` + `kpis.rate.teacher`.
...
- **The two rating settings are school-level data**, read by the same resolver
  that decides who may rate. With *No*, that role's KPIs are hidden from rating
  screens rather than deleted, so turning it back on loses nothing.
- **How a teacher reaches a principal today — and why rule 7b is stricter than
  payroll.** There is no teacher → principal column. On a `single` school the
  one principal covers everyone. On a `multiple` school, `principal_assignments`
  gives each principal a campus (or all), a division name, a list of grade ids
  and tenure dates; a teacher's grades are **derived** as the union of (a) active
  `timetable_entries` → `sections.grade_id`, joined on
  `staff.school_user_id`, and (b) `sections.class_teacher_id` → grade, which
...
The two earlier confirmations were settled in the third pass: **Principal is
...
`/dashboard/admissions/import` asked for a file before anything said what to put
in it. The mapping screen — which names every field, marks the three required
ones and describes each — is **step two**, and the only route to it is uploading
the file you are asking about.

That is worth remembering as a shape, not as a bug: *the explanation was behind
the thing it explained.* It is the same shape as Sprint 27's orphaned route and
Sprint 28's unrendered `STUDENT_FEE_STATUS_DESCRIPTIONS` — complete, correct,
type-checked work no first-time user could reach. Nothing in the repository
greps for it.

### What was built

`GET /api/school/student-imports/sample`, gated on `students.import`, serving a
CSV generated by `lib/student-import-sample.ts` from `IMPORT_FIELDS`. The
choose-a-file step carries the download plus a table naming every column, its
required mark and its description — the table is read off the generated file, so
the screen cannot describe a column the file lacks.

**No static file in `public/`.** A second copy of the column list, outside the
type-checker and outside every check, is the copy that goes stale, and a
`/public` URL is an unauthenticated endpoint on a multi-tenant deployment.

### The two couplings a later session must not break

**1. The headings are aliases.** Each sample column is headed with one of its own
field's `aliases`, so `suggestColumnMap` claims it in the *exact-match* pass
rather than the substring pass, where two fields can compete for one heading.
That is what makes a file built on the sample arrive with all ten columns
matched. Renaming an alias can silently un-match one.

**2. The example rows must survive `validateRow`.** Tightening a validation rule
can turn a sample row invalid, and the symptom — the operator's own file
reporting rows that need fixing — reads as dirty school data.

Both are asserted by `npm run check-import-sample`, which also asserts that the
screen links to the route. It is pure, needs no database, and sits in CI beside
`check-loaders`.

### Two repairs the sample forced into the open

* `gender` and `bFormCnic` had **no `hint`**, so the mapping screen named those
  two fields and said nothing about them — and *B-Form / CNIC* is the one a
  clerk is least sure of. Both have one now.
* `'b-form / cnic'` joined that field's aliases. Without it the sample's own
  heading resolved by substring, which works today and is one alias-list edit
  away from not working.

### QA, and what it changed

Run against the standalone build on the live database as the LGS administrator,
via `scripts/qa-emergency-link.mjs` (see §5bs — *Login as Admin* is still the
wrong seat).

* **The round trip was executed, not reasoned about.** The downloaded file was
  fed back into the importer through the real file input: all ten columns
  auto-matched and the server's dry run wrote `total_rows 3, valid_rows 3,
  invalid_rows 0` to `student_import_batches`. **Nothing was committed**, and the
  QA batch and its three rows were deleted afterwards.
* A **teacher** requesting the download gets `403`. Unauthenticated, middleware
  redirects to sign-in rather than serving the file.
* The BOM was checked **on the wire** (`EF BB BF` via `arrayBuffer()`), not
  through `.text()` — `fetch` strips the BOM when decoding, so a `.text()` check
  reports `false` on a file that has one. Worth knowing before somebody
  "fixes" it.
* **One fix applied:** the required marks were `Badge variant="warning"`. Ten
  amber badges on a screen describing a blank file read as ten problems; they are
  `brand` now.

### The one thing the in-app browser could not do

`document.body.innerText` returned 64 characters on a fully rendered page — the
hidden Browser pane does no layout, and `innerText` is layout-dependent.
`innerHTML` and `querySelectorAll` are accurate there; `innerText` and
screenshots are not. Screenshots came from Playwright. That is the hidden-pane
trap again, wearing a fourth costume — the first three were a blank paint, a
stuck skeleton and a stale bundle.
