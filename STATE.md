...
`feature/sprint-0-auth-hardening`. Sprint 9 (§5n) is on its own worktree branch.
Neither is merged. The Stage-4 worktree branches (`stage-4-state-md-100f15`,
`school-management-system-access-92a218`) are merged and stale — prune them.
**Main branch:** `main` — last commit `d0e7dc0`, in sync with `origin/main`.
Migrations `0000`–`0015` applied (`0015` was applied and verified during
Sprint 0's QA). **`0016` is written but NOT applied** — it is Sprint 9's, and
applying it is a DevOps step. Next free number after it: **`0017`**.
...
Migration **`0016_sprint9_exams.sql`**, **written and not applied.** Six tables.
typecheck, lint and build all green. **Nothing has been seen in a browser and
nothing has touched the live database.**
...
4. **No exam-level grade boundaries, no weighting between terms.** A school
...
5. **The class teacher's remark is a ruled blank on the printed card**, not a
   ...
   prints it yet.
6. **No CSV import of marks.** Sprint 10 builds the import machinery; marks are
   the obvious second customer for it.
7. **Grade boundaries are not versioned.** Editing a scheme's bands changes the
