...
**Last updated:** 2026-08-14 (**Sprint 10.5: A, B, D and the screen-level pass all done — §5z**)

> ▶ **NEXT: print one of each document on real A4.** It is the last thing
> blocking a printed-document sign-off *and* the report-card chart, which must
> not be added to a print template nobody has printed. Sprint 10.5's A, B and D
> are complete and C is wired on five surfaces, on
...
> drawer, `PageHeader` on 49 screens, 1155 hardcoded colours migrated onto the
> palette across 145 files, **every table in the product on the `Table`
> primitive**, and the dashboard charts wired to real aggregates. Read §5z for
> what C still does not cover (exams, and the report card's printed chart) and
> for how the 27-file table rewrite was verified — those checks are worth
> reusing for any future bulk change.
...
| C — dashboard visualisation | ⚠️ **mostly done** — five surfaces wired; exams and the report card are not |
...
**`npm run check-dashboard` executes all eight aggregates against the real
schema**, using a location id that belongs to no tenant — so nothing real is
...
- **Exams** — grade distribution per exam, subject-wise averages, pass rate.
  Not built. The queries are more involved than the others because grade bands
  are per-school and resolved through `lib/grading.ts`, so the distribution has
  to be computed against each school's own scheme rather than fixed
  percentages.
