...
export async function PATCH(request: NextRequest, context: RouteContext) {
  try {
    ...
    if (!isUuid(adminId)) return apiFailure('not_found', 'Super admin not found.', 404);

    const target = await findSuperAdminById(adminId);
    if (target === null) return apiFailure('not_found', 'Super admin not found.', 404);
    ...
    if (target.isOwner) {
      if (!actor.isOwner) {
        return apiFailure('owner_protected', 'Only the platform owner can change the owner’s account.', 403);
      }
      if (body.email !== undefined || body.isActive !== undefined || body.permissions !== undefined) {
        return apiFailure(
          'owner_protected',
      ...
      if (body.password !== undefined) {
        return apiFailure('owner_protected', 'Change your own password from My account.', 409);
      }
    }
    ...
    if (body.email !== undefined) {
      ...
      const problem = email === '' ? 'Enter an email address.' : emailRejectionReason(email);
    ...
    if (body.isActive !== undefined) {
      if (typeof body.isActive !== 'boolean') {
        return apiFailure('invalid_body', 'isActive must be true or false.', 400);
      }
      if (!body.isActive && actor.adminId === target.id) {
        return apiFailure('self', 'You cannot deactivate your own account.', 409);
      }
    ...
    if (body.permissions !== undefined) {
      if (!actor.isOwner) {
        return apiFailure('forbidden', 'Only the platform owner can change permissions.', 403);
      }
...
export async function DELETE(_request: NextRequest, context: RouteContext) {
  try {
    ...
    if (target.isOwner) {
      return apiFailure('owner_protected', 'The platform owner cannot be deleted.', 409);
    }
