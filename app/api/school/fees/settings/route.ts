...
import { DEFAULT_AUTO_GENERATE_DAY } from '@/lib/voucher-auto-generate';
    ...
    try {
      ...
      return apiSuccess({
        settings: rule ?? {
          ...
          // Off, matching the column's own default. A school that has never
          // opened this screen is not having its billing decided for it.
          autoGenerateVouchers: false,
          autoGenerateDay: DEFAULT_AUTO_GENERATE_DAY,
          autoGenerateFamilyVouchers: true,
        },
...
interface UpdateSettingsBody {
  ...
  autoGenerateVouchers?: unknown;
  autoGenerateDay?: unknown;
  autoGenerateFamilyVouchers?: unknown;
}
...
export const PATCH = withSchoolAuth(
  async (request, auth) => {
    try {
      ...
      /*
       * Automatic generation, read the same way and for a sharper reason
       * (Sprint 27, item A5).
       *
       * Absent means **off**. This is a replace and not a merge, the form
       * always sends all three fields, and a body that does not mention
       * `autoGenerateVouchers` must not leave a school silently raising four
       * hundred money demands on the 25th of every month. `auto_send` above
       * emails vouchers a person decided to raise; this one does the deciding.
       *
       * `autoGenerateFamilyVouchers` defaults to **true** rather than false,
       * and that is not an inconsistency: it decides nothing at all until the
       * flag above is on, and a school that has switched automatic billing on
       * has said it wants the month raised the way its clerk would raise it.
       */
      const autoGenerateVouchers = readBoolean(body.autoGenerateVouchers, false);
      const autoGenerateFamilyVouchers = readBoolean(
        body.autoGenerateFamilyVouchers,
        true,
      );

      const autoGenerateDay = Number(body.autoGenerateDay ?? DEFAULT_AUTO_GENERATE_DAY);
      if (
        !Number.isInteger(autoGenerateDay) ||
        autoGenerateDay < 1 ||
        autoGenerateDay > 28
      ) {
        return apiFailure(
          'invalid_body',
          'The generation day must be a whole number between 1 and 28.',
          400,
        );
      }
