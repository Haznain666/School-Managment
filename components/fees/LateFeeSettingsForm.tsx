...
/**
 * `25` → `25th`, for the sentence that explains pre-paid billing.
 *
 * Written here rather than pulled from `Intl`, which has no ordinal formatter
 * that produces the suffix on its own. Eleven to thirteen are the exceptions
...
function ordinal(day: number): string {
  ...
  const units = day % 10;
  if (units === 1) return `${String(day)}st`;
  if (units === 2) return `${String(day)}nd`;
  if (units === 3) return `${String(day)}rd`;
...
export function LateFeeSettingsForm({
  ...
  const [autoGenerate, setAutoGenerate] = useState(initial.autoGenerateVouchers);
  const [autoGenerateDay, setAutoGenerateDay] = useState(
    String(initial.autoGenerateDay),
  );
  const [autoGenerateFamily, setAutoGenerateFamily] = useState(
    initial.autoGenerateFamilyVouchers,
  );
    ...
    try {
      await schoolFetch('/api/school/fees/settings', {
        ...
        body: JSON.stringify({
          ...
          autoGenerateVouchers: autoGenerate,
          autoGenerateDay: Number(autoGenerateDay) || 25,
          autoGenerateFamilyVouchers: autoGenerateFamily,
        }),
      });
  ...
  const generateDay = Number(autoGenerateDay) || 25;
  const dueDayNumber = Number(dueDay) || 10;
  ...
  return (
    <div className="space-y-4">
      ...
      {/*
        Sprint 27, item A6. Raising next month's vouchers on a timer.

        Its own card and deliberately above the late fee one: this is the
        decision that produces a voucher, and the late fee is a decision about
        one that already exists.

        Off by default and it stays off. `auto_send_vouchers` on the Billing
        card emails vouchers a person decided to raise; this one does the
        deciding, and a voucher a parent has already been shown cannot be
      ...
      <Card
        header={
          <CardTitle
            ...
            description="Whether next month's vouchers are raised for you, and when."
          />
        }
      >
        <div className="space-y-5">
          <Toggle
            ...
            disabled={!canEdit}
            label="Generate next month's vouchers automatically"
            description="Raises a voucher for every actively enrolled student in a grade that has a price list. Anyone who already holds a live voucher for that month is skipped."
          />
            ...
            <>
              <div className="sm:max-w-xs">
                <Input
                  ...
                  hint="Capped at the 28th so every month has that day."
                  onChange={(event) => {
                    setAutoGenerateDay(event.target.value);
                  }}
                />
              </div>

              {/*
                The sentence that makes the setting legible.

                Fees here are **pre-paid** — October's fee is billed during
                September — and nothing else on this screen says so. Without
                this line a school reads "generate on the 25th" as "bill the
                25th's month", sets it to the 1st, and is then surprised to be
                billing a month ahead. The due day in it is the school's own,
                read from the field above rather than assumed to be the 10th.
              */}
              <p className="rounded-lg bg-surface-subtle px-3 py-2 text-sm text-content-muted">
                On the {ordinal(generateDay)} of each month, raise every
                student&apos;s voucher for the <strong>following</strong> month.
                A voucher raised on {ordinal(generateDay)} September is for
                October and falls due on {ordinal(dueDayNumber)} October.
              </p>

              <Toggle
                checked={autoGenerateFamily}
                onChange={setAutoGenerateFamily}
                disabled={!canEdit}
                label="Raise one voucher per family"
                description="Where a guardian has more than one child here, the run raises each child's voucher and one family voucher over them — the same slip your clerk would club by hand."
              />
            </>
          ) : null}
        </div>
