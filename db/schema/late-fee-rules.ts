...
export const lateFeeRules = pgTable(
  'late_fee_rules',
  {
    ...
    siblingDiscountForLastChild: boolean('sibling_discount_for_last_child')
      ...
      .default(false),
    /**
     * Whether the school raises next month's vouchers on a timer (Sprint 27).
     *
     * **Off, and it must stay off until a school turns it on.** The same rule
     * as `auto_send_vouchers` above and worse in one respect: that one emails
     * vouchers the school had already decided to raise, while this one decides
     * to raise them. A scheduler that started billing at a school which never
     * asked for it would have put money demands in front of parents on the
     * school's own letterhead, and a voucher a parent has been shown cannot be
     * recalled any more than an email can.
     */
    autoGenerateVouchers: boolean('auto_generate_vouchers').notNull().default(false),
    /**
     * Day of month **M − 1** the run fires. Capped at 28 so every month has it.
     *
     * A school here is pre-paid: October's fee is billed during September and
     * falls due on `due_day` of October. So the 25th means "on the 25th of
     * September, raise every student's October voucher", and
     * `defaultDueDate(10, 2026, dueDay)` dates it for the 10th of October.
     */
    autoGenerateDay: integer('auto_generate_day').notNull().default(25),
    /**
     * The claim column, exactly as `auto_send_last_run_on` above is, and for
     * exactly the same reason: seven server processes, seven timers, and a
     * read-then-check that would let all seven bill the same school.
     *
     * Null means never run. `lib/voucher-auto-generate.ts` moves it to today
     * inside the conditional `UPDATE … RETURNING` that claims the school, and
     * hands it back to null on a throw — a failed run recorded as a run is a
     * month nobody bills.
     */
    autoGenerateLastRunOn: date('auto_generate_last_run_on'),
    /**
     * Whether the automatic run clubs siblings into one family voucher.
     *
     * Default **true**, unlike everything else on this table, and the asymmetry
     * is deliberate: this flag decides nothing until `auto_generate_vouchers`
     * is on, and a school that has switched automatic billing on has said it
     * wants the month raised the way its clerk would raise it — one slip per
     * family. A school that would rather bill every child separately turns it
     * off on `/dashboard/fees/settings`.
     */
    autoGenerateFamilyVouchers: boolean('auto_generate_family_vouchers')
      .notNull()
      .default(true),
  ...
  (table) => [
    ...
    check(
      'late_fee_rules_auto_generate_day_check',
      sql`${table.autoGenerateDay} BETWEEN 1 AND 28`,
    ),
  ],
);
