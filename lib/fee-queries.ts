...
export interface LateFeeSettings {
   ...
   *
   * Off until a school turns it on — see `db/schema/late-fee-rules.ts`, which
   * says why this one is the strictest of the four flags on this row.
   */
  autoGenerateVouchers: boolean;
  /** Day of month **M − 1** the generation runs on. */
  autoGenerateDay: number;
  /** Whether that run clubs siblings into one voucher. Default on. */
  autoGenerateFamilyVouchers: boolean;
  /**
...
export async function getLateFeeRule(
  ...
  const rows = await db
    .select({
      ...
      autoGenerateVouchers: lateFeeRules.autoGenerateVouchers,
      autoGenerateDay: lateFeeRules.autoGenerateDay,
      autoGenerateFamilyVouchers: lateFeeRules.autoGenerateFamilyVouchers,
    })
    .from(lateFeeRules)
