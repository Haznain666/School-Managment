...
Migration `0033` — **NOT YET APPLIED.** Every case marked 🔒 below depends on it
and cannot be executed until it is. See "Status" for what that blocks and why.
...
| 🔒 | **blocked on migration `0033` being applied** |
...
**Why 🔒 exists.** `student_credits` and `fee_challans.credit_applied` do not
exist on the live database yet. Until `0033` is applied, `previewChallan`,
`generateAdmissionChallan` and `repriceOpenChallans` all throw on a missing
relation, so no fee case can be driven. This is the ordering dependency STATE.md
§5aw records, and it is why the migration goes in **before** the merge, not
after.
